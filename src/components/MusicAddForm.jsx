...
const MusicAddForm = () => {
    ...
    async function handleSubmit(formData) {
        ...
        try {
            const result = await AddLyricDetails(formData);
            if (result.success) {
                router.push(`/lyrics/${result.id}`);
            }
        } catch (e) {
            setError('Failed to add music details. Please try again.');
            console.error(e);
        } finally {
    ...
    return (
        <form 
            action={handleSubmit} 
            className="w-full flex flex-col md:flex-row items-start p-16 gap-4 mt-10"
        >
            {error && (
